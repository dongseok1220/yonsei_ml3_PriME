...
class LoraInjectedLinear(nn.Module):
    def __init__(
        ...
        if w_gate:
            ...
            # torch.nn.init.xavier_uniform_(self.input_gate)
...
class Transformer(nn.Module):
    ...
    def get_weighted_lora(self,
                          lora_corpus: list,
                          weights: list):
        # 1d if LoRA-level merge, or 2d if piece-level merge.
        weights = np.array(weights)
        piece_merge = len(weights.shape) == 2

        keys = lora_corpus[0].keys()
        final_dict = defaultdict(float)
        for i, lora_state_dict in enumerate(lora_corpus):
            for key in keys:
                layer_indx = int(re.search("layers\.(\d+)\.", key).groups()[0])
                w = weights[i][layer_indx] if piece_merge else weights[i]
                final_dict[key] += w * lora_state_dict[key].cuda()
    ...
    def get_new_lora(self, lora_path_list: list, 
        ...
        # print(f"lora_param_list: {(lora_param_list)}")
